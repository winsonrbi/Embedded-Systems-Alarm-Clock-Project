...
void DisplayTime (unsigned short val){
	//takes a time in seconds and generates the correct hours minutes and seconds then renders that on the screen
...
void AlarmTimeDisplay(unsigned short val , unsigned char val2){
	//Same as TimeDisplay but displays the miniature alarm time when alarm is set to on
	unsigned char hour1 = val/36000;
	val = val%36000;
	unsigned char hour2 = val/3600;
	val = val%3600;
...
void SMDisplay(){
	//Controls the Display and transfers control of the LCD Panel to the Time Set or Alarm Set when they need to occupy the screen
...
void SMTimer(){
	//The main timer that keeps track of the time and can also take the newTime sent by the TimeSet State Machine and set that as the new time.
...
void SMAlarmSet(){
	//Sets the Alarm when B is pressed on Numpad, the Alarm button cannot be turned on when the alarm is being set, also the alarm time is also remembered by the EEProm in the ATMega 
	static unsigned char i = 0;
...
void SMTimeSet(){
	//Sets the newTime when A is pressed on the Numkey pad
	static unsigned char i = 0;
	static unsigned char hour1,hour2,minute1,minute2,second1,second2 = 0;
	static unsigned short temp;
...
void SMAlarm(){
	//Takes the button and if pressed will turn Alarm On/Off, starts by default in off state as a safety feature, also can not be turned on when setting the Alarm
...
void SMMain(){
	//If Alarm is on this is what compares the Alarm Time with the Current Time and if they are the same the Alarm will go off by setting the motor on and the speaker on
...
void SMMotor(){
	//Controls both motors by sending signals the motor driver
...
void SMPlaySpeaker(){
	//Plays the tune when the Alarm goes off, I would have done this with an Array and a for loop but every time I try it just plays the same note over and over again regardless of the note.
